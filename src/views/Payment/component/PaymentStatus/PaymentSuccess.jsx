...
const PaymentStatus = ({
  ...
  <div className="d-flex justify-content-center align-items-center flex-column Payment_Status_View">
    <div className="mt-5">
      <CImg src={status === paymentFail ? SVG.crossIcon : SVG.tickIcon} />
    </div>
    <div style={{ width: '100%' }}>
      <p className="mt-4 text-center font-weight-bold Font-Size--16px">
        {status}
      </p>
      <p className="mt-2 text-center paraTag">
        {moment(new Date()).format(dateFormat)},{' '}
        {moment(new Date()).format(timeFormat)}
      </p>
      <p className="mt-5 text-center font-weight-bold paraTag">
        {status === paymentFail ? paymentFailMsg : paymentSuccessfulMsg}
      </p>
      <div className="d-flex justify-content-between align-items-center information_View">
        <div className="d-flex flex-column">
          ...
          <p
            className={`paraTag font-weight-bold text-capitalize ${
              status !== paymentFail ? 'Green' : 'Red'
            }`}
          >
            {planDuration === 'year' ? yearly : monthly}
          </p>
        </div>
        <div className="d-flex align-items-center">
          ...
          <div>
          <div
            className={`Font-Size--22px font-weight-bold align-items-center align-self-center ${
              status !== paymentFail ? 'Green' : 'Red'
            }`}
          >
            {amount}
